...
let substitute_expression_in_normalized_formula (formula: normal_form) (changing_expr: ArithmeticExpression.t) (changed_id: identifier) =
  ...
  in
  let rec substitute_expression_in_formula (formula: Formula.t) (changing_expr: ArithmeticExpression.t) (changed_id: identifier) (renamed_equal_formula: Formula.t) =
    ...
    | Allocation(id, expr) ->
      ...
      if id = changed_id then
        ...
        | None  ->
          let subformula = annotate (Formula.Allocation(changed_id, expr)) formula.annotation in
          update_formula formula (Formula.And(subformula, renamed_equal_formula)), true
      else
    ...
    | NonAllocated(id) ->
      if id = changed_id then
        ...
        | None  ->
          let subformula = annotate (Formula.NonAllocated(changed_id)) formula.annotation in
          update_formula formula (Formula.And(subformula, renamed_equal_formula)), true
      else
        formula, false
    ...
    | And(lformula, rformula) ->
      let lformula, lrenamed = substitute_expression_in_formula lformula changing_expr changed_id renamed_equal_formula in
      let rformula, rrenamed = substitute_expression_in_formula rformula changing_expr changed_id renamed_equal_formula in
  ...
  let renamed_equal_formula = annotate (Formula.Comparison(BinaryComparison.Equals, changing_expr, id_expr)) changing_expr.annotation in
  let substitute_in_disjoint (formula: Formula.t) (disjoints: Formula.t list) (accum_renamed_used: bool) =
    let formula, renamed_used = substitute_expression_in_formula formula changing_expr changed_id renamed_equal_formula in
